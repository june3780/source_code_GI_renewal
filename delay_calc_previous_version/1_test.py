...
def temp_for_file(nets):
    All=copy.deepcopy(nets)

    tt=int()
    while True:
        cell_index=int()
        pin_index=int()
        for idx in range(len(All['temporary_net'+str(tt)])):
            if 'cell_name' in All['temporary_net'+str(tt)][idx]:
                cell_index=idx
            elif 'net_name' in All['temporary_net'+str(tt)][idx]:
                pin_index=idx
        All['temporary_net'+str(tt)][pin_index]['pin_pos']=[All['temporary_net'+str(tt)][cell_index]['port_pos'][0],All['temporary_net'+str(tt)][cell_index]['port_pos'][1],All['temporary_net'+str(tt)][pin_index]['pin_pos'][2]]
        tt=tt+1
        if tt==3:
            break
