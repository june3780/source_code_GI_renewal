...
def get_parsing_test_cell(address):

    text_keyword='test_cell'
    location_of_lib=address.split('.lib')[0]
    if 'cell_index.json' in os.listdir(location_of_lib):
        for cell in os.listdir(location_of_lib+'/cell/'):
            if cell.endswith('.txt'):
...
def get_lib_info_for_delay_v2(address):
    ...
    #### parsing한 lib파일의 파일명과 같은 디렉토리에 lib_dict_for_nldm_delay_v2.json으로 저장
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v2.json','w') as fw:
        json.dump(nldm_v1,fw,indent=4)
    fw.close()
    return 0
...
def get_lib_info_for_delay_v3(address):
    #### 해당 lib 파일의 디렉토리 위치 : location_of_lib
    location_of_lib=address.split('.lib')[0]
    
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v2.json','r') as fw:
        nldm_v1=json.load(fw)
    fw.close()
    ...
    #### parsing한 lib파일의 파일명과 같은 디렉토리에 lib_dict_for_nldm_delay_v3.json으로 저장
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v3.json','w') as fw:
        json.dump(nldm_v1,fw,indent=4)
    fw.close()
    return 0
...
def get_lib_info_for_delay_v4(address):
    #### 해당 lib 파일의 디렉토리 위치 : location_of_lib
    location_of_lib=address.split('.lib')[0]
    
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v3.json','r') as fw:
        nldm_v1=json.load(fw)
    fw.close()
    ...
    #### parsing한 lib파일의 파일명과 같은 디렉토리에 lib_dict_for_nldm_delay_v3.json으로 저장
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v4.json','w') as fw:
        json.dump(nldm_v1,fw,indent=4)
    fw.close()
...
def get_lib_info_for_delay_v5(address):
    #### 해당 lib 파일의 디렉토리 위치 : location_of_lib
    location_of_lib=address.split('.lib')[0]
    
    with open(location_of_lib+'/lib_dict_for_nldm_delay_v4.json','r') as fw:
        nldm_v1=json.load(fw)
    fw.close()
    ...
    for cell in nldm_v1:
        for pin in nldm_v1[cell]:
            ...
            for case in nldm_v1[cell][pin]:
                ...
                elif case.startswith('case_'):
                    continue

                else:
                    print(case)
            #print(nldm_v1[cell][pin].keys())
        #print(nldm_v1[ivalue].keys())
...
def get_one_info(target_address,target_o_info,target_n_info,keyword):
    ...
    for idx in range(len(real_lines)):
        bre=''
        #### real_line에 idx번째 문자열이 target_o_info 안의 요소일 때는 ':'부터 ';'까지의 내용을 저장
        ...
        #### real_line에 idx번째 문자열이 target_n_info 안의 요소일 때는 '()'안의 내용을 저장
...
if __name__=="__main__":
    ...
    #### 원하는 구간 정보를 txt로 저장 : cell_rise, cell_fall, rise_transition, fall_transition
        ...
        get_parsing_latch(lib_address)
        get_parsing_ff(lib_address)
        get_parsing_test_cell(lib_address)
...
############################################## 다른 파일로 코드 짜기
        ...
        get_lib_info_for_delay_v3(lib_address)

    elif sys.argv[1]=='28':
        get_lib_info_for_delay_v4(lib_address)

    elif sys.argv[1]=='29':
        get_lib_info_for_delay_v5(lib_address)
